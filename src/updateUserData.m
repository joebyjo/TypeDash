...
function updateUserData(user,data)
    ...
    %{
    update user stats and appends game data to games.csv
    %}
    ...
    % checking whether data needs to be updated within user stats
    if netWPM_ > stats(2)
        stats(2) = netWPM_;
    end
    ...
    % get only users games ,which is then used to find averages
    userGames = [];
    allGameData = getAllData(games);
    ...
    stats(5) = round(mean(cellfun(@str2double, userGames(:, 7))),1);
    stats(6) = round(mean(cellfun(@str2double, userGames(:, 5))),1);
    stats(7) = round(mean(cellfun(@str2double, userGames(:, 8))),1);
